...
> **兼容性**：Hermes Agent **≥ v0.18.0**（依赖 `cron.jobs.mark_job_run` 收尾函数）。已实测 v0.18.2 (2026.7.7.2)。
...
失败时发送纯文本告警到飞书（2026-08 修复：卡片在飞书会触发 99992402 校验失败，改用纯文本）：

```
🟠 Cron 任务推送失败: 每日数据备份
  任务: 每日数据备份
  Job ID: abc123def456
  时间: 2026-01-01 08:00:00
  错误: ...
  请检查日志: ~/.hermes/logs/agent.log
```
